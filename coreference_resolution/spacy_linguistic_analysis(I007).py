...
def main():
    ...
    with open(file_path) as tsv:
        ...
        for line in csv.reader(tsv, delimiter="\t"):
            ...
            for i in text.split("\n"):
                ...
                # print(text)
                nouns, adjectives, prepositions = pos_tagging(text)
    ...
    convert_to_trec(file_name, all_na, 'I007')
...
def pos_tagging(sentence):
    ...
    return nouns, adjectives, prepositions
...
def convert_to_trec(file_name, all_nouns, export):
    ...
    with open(file_path) as tsv:
        ...
        for line in csv.reader(tsv, delimiter="\t"):
            ...
            for i in text.split("\n"):
                text = re.sub(r"[^a-zA-Z0-9]+", ' ', i)
                if int(num.split('_')[1]) > 3:
                    for n in all_nouns[j-1]:
                        nouns = nouns + ' ' + n
                    if all_nouns[j-1]:
                        noun_str = '0.75 #combine(' + nouns + ')'
                    nouns = ''
                    for n in all_nouns[j-2]:
                        nouns = nouns + ' ' + n
                    if all_nouns[j-2]:
                        noun_str = noun_str + ' 0.5 #combine(' + nouns + ')'
                    nouns = ''
                    for n in all_nouns[j-3]:
                        nouns = nouns + ' ' + n
                    if all_nouns[j-3]:
                        noun_str = noun_str + ' 0.25 #combine(' + nouns + ')'
                elif int(num.split('_')[1]) > 2:
                    for n in all_nouns[j-1]:
                        nouns = nouns + ' ' + n
                    if all_nouns[j-1]:
                        noun_str = '0.75 #combine(' + nouns + ')'
