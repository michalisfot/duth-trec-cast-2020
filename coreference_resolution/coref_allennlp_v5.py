    ...
    for cluster in results["clusters"]:
        ...
        for entity in cluster[1:]:
            ...
            if len(" ".join(temp[slice(entity[0] + offset, entity[1] + offset + 1)])) > 2 and \
                    contains_entity(entity_resol_model, " ".join(temp[slice(entity[0] + offset, entity[1] + offset + 1)])):
                continue
...
def main():
    ...
    output = open("/home/michalis/Desktop/CAsT/testing/train/queries/coref_resol/allennlp_resolved_v5.txt", "w")
    output.write(out)
    output.close()
